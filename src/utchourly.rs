...
impl<Kind: FmtInfo> UtcHourly<Kind> {
...
impl<Kind: FmtInfo> UtcDailyBoundary for UtcHourly<Kind> {
...
impl<Kind: FmtInfo> UtcHourlyBoundary for UtcHourly<Kind> {
    fn get_last_hour(&mut self) -> Option<&str> {
        self.last_hour.as_deref()
    }
...
impl<T: Serialize> UtcHourly<Json<T>> {
...
mod tests {
    ...
    #[test]
    fn test_store() {
        let dir = "test_store_utchourly";
        std::fs::remove_dir_all(dir).unwrap_or_default();
        let mut store = UtcHourly::<Binary>::new(dir.into(), true, "testprogram").unwrap();
        let data = vec![1, 2, 3, 4, 5];
        let now = Utc::now();
        ...
        let _ = store
            ...
            .unwrap();
        let mut store = UtcHourly::<Json<&[u8]>>::new(dir.into(), true, "testprogram").unwrap();
        let _ = store.store(now, &data.as_ref()).unwrap();
        let _ = store
            .store(now + Duration::hours(2), &data.as_ref())
            .unwrap();
    ...
    #[test]
    fn test_string() {
        let dir = "test_string_utchourly";
        std::fs::remove_dir_all(dir).unwrap_or_default();
        let data = "Hello\nworld".to_string();
        let mut store = UtcHourly::<Json<&str>>::new(dir.into(), true, "testprogram").unwrap();
        let now = Utc::now();
